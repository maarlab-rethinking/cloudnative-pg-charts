...
![Version: 0.8.1-maarlab.2](https://img.shields.io/badge/Version-0.8.1--maarlab.2-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square)
...
| cluster.monitoring.prometheusRule.additionalLabels | object | `{}` | Additional labels to add to all alerts generated by the PrometheusRule. |
...
| cluster.postgresql.extensions | list | `[]` | Extensions to mount from container images, via the `image volume extensions` feature (requires CloudNativePG >= 1.27, PostgreSQL >= 18 and Kubernetes >= 1.33). Each entry is passed through verbatim to `spec.postgresql.extensions`; see https://cloudnative-pg.io/documentation/current/imagevolume_extensions/ |
...
| cluster.projectedVolumeTemplate | object | `{}` | Mount custom files in the PostgreSQL Pods through `spec.projectedVolumeTemplate`. See: https://cloudnative-pg.io/documentation/current/cluster_conf/#projected-volumes |
